...
class ActionViewSetTest(TestCase):
    ...
    def test_tweet_filter(self):
        ...
        handle = Handle.objects.get(screen_name='Test handle')
        tweet = mommy.make(Tweet, handle=handle)

        response = self.client.get(url, {'tweet_id': tweet.id})

        self.assertEqual(response.data['count'], 1)
        self.assertEqual(response.data['results'][0]['tweet'], tweet.id)
